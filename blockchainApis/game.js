...
const syncPoolCache = async () => {
    const handleSync = async () => {
        try {
            ...
            poolCaches = poolCaches.filter(poolCache => Number(poolCache.cacheBalance)!=0);
            var game_ids = poolCaches.map(poolCache => poolCache.game_id).slice(0, 50);
            var cacheBalances = poolCaches.map(poolCache => Number(poolCache.cacheBalance)).slice(0, 50);
            var reserveBalance = cacheBalances.map(cacheBalance => -1 * cacheBalance);

            // game win - cachBalance < 0 (insight from pool balance)
            var winStates = cacheBalances.map(cacheBalance => cacheBalance < 0);
            var cacheAmount = cacheBalances.map(cacheBalance => String(Math.abs(cacheBalance)));
            ...
            var tx = await routerContract.batchGameUpdate(game_ids, cacheAmount, winStates)
                .catch((err) => {
                    console.log(err);
                    poolCacheController.updates({
                        game_ids: game_ids,
